...
namespace chess {
    ...
    public:
        ...
        constexpr inline void shift() {
            m_squares = _shift_internal<d>(m_squares);
        }

        template <Direction d>
        constexpr inline Bitboard shift() const {
        ...
        constexpr inline bool isSingleBit() const {
            return m_squares && !(m_squares & (m_squares - 1));
        }

        constexpr inline bool isMultipleBits() const {
            return !isSingleBit();
        }
    ...
    public:
        constexpr BitboardIterator(const Bitboard& bb) : m_bb(bb) {
        }

        constexpr bool hasNext() const {
            return m_bb.nonEmpty();
        }

        constexpr Square next() {
            return m_bb.poplsb();
        }
